...
void parse_window(xml_node<>* window_node, Window* window) {
	...
	if (width) {
		window->width = atoi(node->value());
	}
	if (height) {
		window->height = atoi(node->value());
	}
}

void parse_camera(xml_node<>* camera_node, Camera* camera) {
	xml_node<>* node, * p1, * p2, * p3, * l1, * l2, * l3, * u1, * u2, * u3, * pro1, * pro2, * pro3;
	...
	xml_node<>* projection = camera_node->first_node("projeciton");
	if (pos) {
		if (node = camera_node->first_node("position")) {
			if (p1 = node->first_node("x")) {
				camera->pos[0] = atof(p1->value());
			}
			if (p2 = node->first_node("y")) {
				camera->pos[1] = atof(p2->value());
			}
			if (p3 = node->first_node("z")) {
	...
	if (lookat) {
		if (node = camera_node->first_node("lookat")) {
			if (l1 = node->first_node("x")) {
				camera->lookAt[0] = atof(l1->value());
			}
			if (l2 = node->first_node("y")) {
				camera->lookAt[1] = atof(l2->value());
			}
			if (l3 = node->first_node("z")) {
	...
	if (up) {
		if (node = camera_node->first_node("up")) {
			if (u1 = node->first_node("x")) {
				camera->up[0] = atof(u1->value());
			}
			if (u2 = node->first_node("y")) {
				camera->up[1] = atof(u2->value());
			}
			if (u3 = node->first_node("z")) {
	...
	if (projection) {
		if (node = camera_node->first_node("projection")) {
			if (pro1 = node->first_node("fov")) {
				camera->projection[0] = atof(pro1->value());
			}
			if (pro3 = node->first_node("near")) {
				camera->projection[1] = atof(pro3->value());
			}
			if (pro3 = node->first_node("far")) {
